...
my $site = create_site($schema, '0gall1');
...

{
    ...
    $mech->content_contains('amw-anchors.js');
    foreach my $id (qw/first second third/) {
        $mech->content_contains(qq{#text-amuse-label-$id});
        $mech->content_contains(qq{id="text-amuse-label-$id"});
    }
