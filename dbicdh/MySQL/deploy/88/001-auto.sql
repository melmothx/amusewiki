...
-- Created on Fri Jul 31 11:58:23 2026
--
;
...
CREATE TABLE `custom_formats` (
  ...
  `bb_body_only` smallint NULL DEFAULT 0,
...
CREATE TABLE `node` (
  ...
  INDEX `node_site_id_sorting_pos_uri_index` (`site_id`, `sorting_pos`, `uri`),
  PRIMARY KEY (`node_id`),
...
CREATE TABLE `oai_pmh_set` (
  `oai_pmh_set_id` integer NOT NULL auto_increment,
  `set_spec` varchar(255) NOT NULL,
  `site_id` varchar(16) NOT NULL,
  `set_name` text NULL,
  INDEX `oai_pmh_set_idx_site_id` (`site_id`),
  PRIMARY KEY (`oai_pmh_set_id`),
  UNIQUE `set_spec_site_id_unique` (`set_spec`, `site_id`),
...
CREATE TABLE `title` (
  ...
  `text_qualification` varchar(32) NULL,
...
CREATE TABLE `whitelist_ip` (
  ...
  `granted_by_username` varchar(255) NULL,
  `expire_epoch` integer NULL,
  INDEX `whitelist_ip_idx_site_id` (`site_id`),
  INDEX `whitelist_ip_ip_amw_index` (`ip`),
...
CREATE TABLE `oai_pmh_record` (
  `oai_pmh_record_id` integer NOT NULL auto_increment,
  ...
  `metadata_format_description` varchar(255) NULL,
  `deleted` integer(1) NOT NULL DEFAULT 0,
  `update_run` integer NOT NULL DEFAULT 0,
  INDEX `oai_pmh_record_idx_aggregation_id` (`aggregation_id`),
  INDEX `oai_pmh_record_idx_aggregation_series_id` (`aggregation_series_id`),
  ...
  UNIQUE `identifier_site_id_unique` (`identifier`, `site_id`),
  CONSTRAINT `oai_pmh_record_fk_aggregation_id` FOREIGN KEY (`aggregation_id`) REFERENCES `aggregation` (`aggregation_id`) ON DELETE SET NULL ON UPDATE CASCADE,
  CONSTRAINT `oai_pmh_record_fk_aggregation_series_id` FOREIGN KEY (`aggregation_series_id`) REFERENCES `aggregation_series` (`aggregation_series_id`) ON DELETE SET NULL ON UPDATE CASCADE,
...
CREATE TABLE `oai_pmh_record_set` (
  ...
  CONSTRAINT `oai_pmh_record_set_fk_oai_pmh_record_id` FOREIGN KEY (`oai_pmh_record_id`) REFERENCES `oai_pmh_record` (`oai_pmh_record_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `oai_pmh_record_set_fk_oai_pmh_set_id` FOREIGN KEY (`oai_pmh_set_id`) REFERENCES `oai_pmh_set` (`oai_pmh_set_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB;
