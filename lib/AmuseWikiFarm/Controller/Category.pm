...
sub author :Path('/authors') :Args(1) {
    my ( $self, $c, $arg ) = @_;
    $c->res->body($c->uri_for($c->action, $arg));
}
...
sub category_listing :Private {
    ...
    # manipulate the full uri
    foreach my $el (@$list) {
        $el->{uri} = $c->uri_for_action("/category/$type", $el->{uri});
    }
    # stash
    $c->stash(list => $list);
    $c->stash(template => 'category.tt');
}
